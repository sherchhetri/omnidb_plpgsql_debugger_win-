/**********************************************************************
 * OmniDB PL/pgSQL Debugger (Windows/MSVC-ready)
 **********************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#ifdef _MSC_VER
 /* MSVC: silence 'unsafe' CRT warnings for sprintf, etc. */
...
#include "plpgsql.h"
#include "miscadmin.h"
#include "executor/spi.h"     /* SPI_gettype/SPI_getvalue */
#include "libpq-fe.h"
#include "fmgr.h"
...
/**********************************************************************
 * Prototypes (exported + internal)
 **********************************************************************/
PGDLLEXPORT void _PG_init(void);
PGDLLEXPORT void load_plugin(PLpgSQL_plugin* hooks);
PGDLLEXPORT Datum omnidb_enable_debugger(PG_FUNCTION_ARGS);
...
static PLpgSQL_plugin plugin_funcs = {
  profiler_init, profiler_func_beg, profiler_func_end, profiler_stmt_beg, profiler_stmt_end
};
PGconn* omnidb_plugin_conn;
bool omnidb_plugin_active = false;
unsigned int omnidb_plugin_depth = (unsigned int)-1;  /* was -1 */
...
/* SQL-callable function (exported) */
PG_FUNCTION_INFO_V1(omnidb_enable_debugger);
PGDLLEXPORT Datum omnidb_enable_debugger(PG_FUNCTION_ARGS)
{
    omnidb_plugin_active = true;
#ifdef _MSC_VER
    /* Use secure copy on MSVC; sprintf below is silenced via pragma */
    strcpy_s(omnidb_plugin_conninfo, sizeof(omnidb_plugin_conninfo),
        text_to_cstring(PG_GETARG_TEXT_P(0)));
#else
    snprintf(omnidb_plugin_conninfo, sizeof(omnidb_plugin_conninfo), "%s",
        text_to_cstring(PG_GETARG_TEXT_P(0)));
#endif
    PG_RETURN_VOID();
}
    ...
    if (omnidb_plugin_active)
    {
        ...
        if (omnidb_plugin_depth == 0)
        {
            ...
            if (PQstatus(omnidb_plugin_conn) != CONNECTION_BAD)
            {
                ...
                sprintf(select_context,
                    "SELECT pid FROM omnidb.contexts WHERE pid = %i", MyProcPid);
                PGresult* res = PQexec(omnidb_plugin_conn, select_context);
                if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) == 1)
                {
                    char update_context[1024];
                    sprintf(update_context,
                        "UPDATE omnidb.contexts SET function = '%s', hook = 'func_beg', "
                        "stmttype = 'BEGIN', lineno = NULL where pid = %i",
                        findProcName(func->fn_oid), MyProcPid);
            ...
            {
                omnidb_plugin_active = false;
                elog(ERROR, "omnidb: Connection to database failed: %s",
                    PQerrorMessage(omnidb_plugin_conn));
            }
        }
        else
        {
...
static void profiler_func_end(PLpgSQL_execstate* estate, PLpgSQL_function* func)
{
    ...
    if (omnidb_plugin_active && !omnidb_plugin_depth)
    {
        ...
        sprintf(update_context,
            "UPDATE omnidb.contexts SET finished = true WHERE pid = %i",
            MyProcPid);
        PQexec(omnidb_plugin_conn, update_context);

        char unlock[256];
        sprintf(unlock,
            "SELECT pg_advisory_unlock(%i) FROM omnidb.contexts WHERE pid = %i",
            MyProcPid, MyProcPid);
...
{
#ifdef DEBUG
    elog(LOG, "omnidb, STMT START, line %d, type %s, %s, %i",
        stmt->lineno, decode_stmt_type(stmt->cmd_type),
        findProcName(estate->func->fn_oid), MyProcPid);
#endif

    if (omnidb_plugin_active && !omnidb_plugin_depth)
    {
        char select_breakpoint[256];
        sprintf(select_breakpoint,
            "SELECT breakpoint FROM omnidb.contexts WHERE pid = %i",
            MyProcPid);
        ...
        if ((omnidb_plugin_breakpoint == 0 || omnidb_plugin_breakpoint == stmt->lineno) &&
            ((stmt->cmd_type == 11 && stmt->lineno != 0) || (stmt->cmd_type != 11)))
        {
            update_variables(estate);

            char update_context[1024];
            sprintf(update_context,
                "UPDATE omnidb.contexts SET function = '%s', hook = 'stmt_beg', "
                "stmttype = '%s', lineno = %d where pid = %i",
                findProcName(estate->func->fn_oid),
                decode_stmt_type(stmt->cmd_type),
                stmt->lineno, MyProcPid);
            PQexec(omnidb_plugin_conn, update_context);
            ...
            char lock[256];
            sprintf(lock,
                "SELECT pg_advisory_lock(%i) FROM omnidb.contexts WHERE pid = %i",
                MyProcPid, MyProcPid);
            PQexec(omnidb_plugin_conn, lock);
        }

        char insert_statistics[256];
        sprintf(insert_statistics,
            "INSERT INTO omnidb.statistics (pid, lineno, step, tstart, tend) "
            "VALUES (%i, %i, %i, now(), NULL)",
            MyProcPid, stmt->lineno, omnidb_plugin_step);
...
{
#ifdef DEBUG
    elog(LOG, "omnidb, STMT END, line %d, type %s, %s, %i",
        stmt->lineno, decode_stmt_type(stmt->cmd_type),
        findProcName(estate->func->fn_oid), MyProcPid);
#endif
    ...
    if (omnidb_plugin_active && !omnidb_plugin_depth)
    {
        ...
        sprintf(update_statistics,
            "UPDATE omnidb.statistics SET tend = now() "
            "WHERE pid = %i AND lineno = %i AND step = %i",
            MyProcPid, stmt->lineno, omnidb_plugin_step);
...
char* decode_stmt_type(int typ)
{
    ...
    switch (typ)
    {
    case PLPGSQL_STMT_BLOCK:        decoded_type = "BLOCK"; break;
    case PLPGSQL_STMT_ASSIGN:       decoded_type = "ASSIGN"; break;
    case PLPGSQL_STMT_PERFORM:      decoded_type = "PERFORM"; break;
    case PLPGSQL_STMT_GETDIAG:      decoded_type = "GETDIAG"; break;
    case PLPGSQL_STMT_IF:           decoded_type = "IF"; break;
    case PLPGSQL_STMT_CASE:         decoded_type = "CASE"; break;
    case PLPGSQL_STMT_LOOP:         decoded_type = "LOOP"; break;
    case PLPGSQL_STMT_WHILE:        decoded_type = "WHILE"; break;
    case PLPGSQL_STMT_FORI:         decoded_type = "FORI"; break;
    case PLPGSQL_STMT_FORS:         decoded_type = "FORS"; break;
    case PLPGSQL_STMT_FORC:         decoded_type = "FORC"; break;
    case PLPGSQL_STMT_EXIT:         decoded_type = "EXIT"; break;
    case PLPGSQL_STMT_RETURN:       decoded_type = "RETURN"; break;
    case PLPGSQL_STMT_RETURN_NEXT:  decoded_type = "RETURN NEXT"; break;
    case PLPGSQL_STMT_RETURN_QUERY: decoded_type = "RETURN QUERY"; break;
    case PLPGSQL_STMT_RAISE:        decoded_type = "RAISE"; break;
    case PLPGSQL_STMT_EXECSQL:      decoded_type = "EXEC SQL"; break;
    case PLPGSQL_STMT_DYNEXECUTE:   decoded_type = "DYNEXECUTE"; break;
    case PLPGSQL_STMT_DYNFORS:      decoded_type = "DYNFORS"; break;
    case PLPGSQL_STMT_OPEN:         decoded_type = "OPEN"; break;
    case PLPGSQL_STMT_FETCH:        decoded_type = "FETCH"; break;
    case PLPGSQL_STMT_CLOSE:        decoded_type = "CLOSE"; break;
    }
    return decoded_type;
}
...
static char* get_text_val(PLpgSQL_var* var, char** name, char** type)
{
    ...
    text_value = DatumGetCString(FunctionCall3(&finfo_output,
        var->value,
    ...
    if (name) *name = var->refname;
    if (type) *type = var->datatype->typname;
...
static void update_variables(PLpgSQL_execstate* estate)
{
    ...
    for (int i = 0; i < estate->ndatums; i++)
    {
        ...
        case PLPGSQL_DTYPE_VAR:
        {
            ...
            char* typeName = var->datatype ? var->datatype->typname : "InvalidType";
            ...
            sprintf(insert_variable,
                "INSERT INTO omnidb.variables (pid, name, attribute, vartype, value) "
                "VALUES (%i, '%s', NULL, '%s', '%s')",
                MyProcPid, name, typeName, val);
                ...
                if (tupdesc != NULL)
                {
                    for (att = 0; att < tupdesc->natts; ++att)
                    {
                        ...
                        sprintf(insert_variable,
                            "INSERT INTO omnidb.variables (pid, name, attribute, vartype, value) "
                            "VALUES (%i, '%s', '%s', '%s', '%s')",
                            MyProcPid, rec->refname,
                            NameStr(tupdesc->attrs[att].attname), typeName, val);
                        PQexec(omnidb_plugin_conn, insert_variable);
            ...
            if (rec->tupdesc != NULL)
            {
                for (att = 0; att < rec->tupdesc->natts; ++att)
                {
                    ...
                    sprintf(insert_variable,
                        ...
                        "VALUES (%i, '%s', '%s', '%s', '%s')",
                        MyProcPid, rec->refname,
                        NameStr(rec->tupdesc->attrs[att]->attname), typeName, val);
                    PQexec(omnidb_plugin_conn, insert_variable);
